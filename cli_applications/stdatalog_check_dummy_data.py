...
# ******************************************************************************
"""
This script, `stdatalog_check_dummy_data.py`, is designed to check for dummy data in acquisition
folders generated by STMicroelectronics' HSDatalog tool.
It supports various options for customizing the data analysis, including selecting specific
sensors, setting time ranges, and more.
...
def hsd_dataframe(acq_folder, sensor_name, start_time, end_time, custom_device_model, chunk_size):
    """Run dummy-data checks for one or more components.

    Parameters:
    - acq_folder: str path to the acquisition folder.
    - sensor_name: str sensor/component name or `all` for all active.
    - start_time: int start time (seconds) for analysis interval.
    - end_time: int end time (seconds) for analysis interval; `-1` means full.
...
def check_data(hsd, component, start_time, end_time, acq_folder, chunk_size):
    """Check dummy data for a specific component.

    Parameters:
    - hsd: HSDatalog instance.
    - component: object representing the component to analyze.
