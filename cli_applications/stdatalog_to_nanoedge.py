...
sensors, setting time ranges, specifying segment length and increment, and more.
...
def validate_signal_increment(ctx, param, value):
    """Validate that the signal increment is non-negative.

    Parameters
    ----------
    ctx : click.Context (unused. Maintained for Click callback signature)
        The Click context.
    param : click.Option (unused. Maintained for Click callback signature)
        Option that provided the value.
    value : int
        Proposed signal increment.

    Returns
    -------
    int
        The validated non-negative increment.
    """
    _ = ctx  # Unused parameter
    _ = param  # Unused parameter
    if value < 0:
        raise click.BadParameter(f"\'{value}\'. It must be a positive value. Please retry")
    return value
...
def hsd_dataframe(
    ...
    """Convert acquisition data to NanoEdge AI Studio dataset format.
    ...
    sensor_name : str
        Sensor name or 'all' to process all active sensors.
    signal_length : int
        Segment length used during segmentation.
    signal_increment : int
        Step between segments (controls overlap). 0 means no overlap.
    start_time : int
    ...
    target_value : float | None
        Optional target value for NEAI extrapolation datasets.
    custom_device_model : tuple[int, int, str] | None
        Optional custom DTDL (board_id, fw_id, json_path).
    debug : bool
        Enable timestamp recovery checks and debugging.
    """
    ...
    while df_flag:
        ...
        if sensor_name == '':
            ...
            if component is not None:
                ...
                convert_data(hsd, component, signal_length, signal_increment, start_time, end_time,
                            raw_data, output_folder, acq_folder, target_value)
            else:
        ...
        # If a specific sensor name is provided, process only that component
            ...
            if component is not None:
                ...
                log.exception('No "%s" Component found in your' \
...
def convert_data(
    ...
    """Convert a component's data to NanoEdge AI Studio format.
    ...
    component : dict
        Component descriptor to convert.
    signal_length : int
        Segment length used during segmentation.
    signal_increment : int
        Step between segments (controls overlap).
    start_time : int
        Start time in seconds for conversion.
    end_time : int
    ...
    acq_folder : str
        Acquisition folder path for error messages.
    target_value : float | None
        Optional target value for extrapolation datasets.
    """
    try:
        # Attempt to convert data to NanoEdge format
        HSDatalog.convert_dat_to_nanoedge(hsd, component, signal_length, signal_increment,
                                        start_time, end_time, raw_data, output_folder,
                                        target_value)
