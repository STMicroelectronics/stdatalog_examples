...
- Export data in different formats (TXT, CSV, TSV, PARQUET, HDF5(*)).
    -- HSDF5 format:
        - acquisition_metadata group: Contains the acquisition information (9 attributes)
            - tags group: [Optional] Contains the acquisition 
                tag list (start_time, end_time, tag_label)
        ...
        - sensor_data group: Contains the sensor data (one group per sensor)
            - sensor_name group: Contains the sensor data (N + 1 datasets: N x axis_data,
            1 x timestamps, a list of sensor-category specific attributes)
...
(*) NOTE for HDF5 format:
    - To view the contents of an HDF5 file, you can:
        -- Use the `stdatalog_hdf5_viewer.py` script provided in the SDK
            stdatalog_examples/cli_applications folder.
        -- If you are a VSCode user, you can install the `HDF5 Viewer` extension to view
            the contents of an HDF5 file directly in the editor.
        -- Use an online HDF5 file viewing service with more features,
...
from stdatalog_core.HSD_utils.exceptions import DataCorruptedException, MissingDeviceModelError, \
    MissingTagsException, MissingISPUOutputDescriptorException
...
def show_help(ctx, param, value):
    ...
    Behavior:
    - Prints command help and a set of curated usage examples.
...
def hsd_data_export(
    ...
    """Export acquisition data in the selected format.

    Parameters:
    - acq_folder: str path to the acquisition folder.
    - output_folder: str | None destination folder (auto-created if missing).
    - file_format: str output format (`TXT`, `CSV`, `TSV`, `PARQUET`, `HDF5`).
    - sensor_name: str sensor/component name; use `all` for all active.
    - start_time: int start time (seconds) for export interval.
    - end_time: int end time (seconds) for export interval; `-1` means full.
    - labeled: bool include annotations if present.
    - tag_labels: tuple[str] optional tag labels filter.
    - no_timestamps: bool remove timestamps column in exported files.
    - raw_data: bool export raw values (not multiplied by sensitivity).
    ...
    while df_flag:
        ...
        if sensor_name == '':
            ...
            if component is not None:
                convert_data(hsd, component, start_time, end_time, acq_folder, labeled,
                            output_folder, file_format, which_tags, no_timestamps, raw_data,
                            chunk_size)
            else:
                break
        # If 'all' is specified for sensor name, process all active components
            ...
            if file_format.upper() == "HDF5":
                convert_data(hsd, component_list, start_time, end_time, acq_folder, labeled,
                            output_folder, file_format, which_tags, no_timestamps, raw_data,
                            chunk_size)
            else:
...
def convert_data(
    ...
    """Convert data for the given component(s) and export to disk.

    Parameters:
    - hsd: HSDatalog instance.
    - components: object | list of objects representing components.
    ...
    - no_timestamps: bool remove timestamps column.
    - raw_data: bool export raw values.
    ...
    try:
        if file_format == "HDF5":
            ...
            HSDatalog.convert_acquisition_to_hdf5(hsd, components, start_time, end_time, labeled,
                                                output_folder, raw_data, which_tags, no_timestamps,
                                                chunk_size)
        else:
            # Attempt to convert data to the specified file format (TXT, CSV, TSV, PARQUET or HDF5)
            HSDatalog.convert_dat_to_xsv(hsd, components, start_time, end_time, labeled,
                                        raw_data, output_folder, file_format, which_tags,
                                        no_timestamps, chunk_size)
