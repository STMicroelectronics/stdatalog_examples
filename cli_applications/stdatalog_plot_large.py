...
NOTEs:
- The ensure out-of-core plots for large datasets, the script, starting from an
    acquisition folder, converts each selected sensor data to a Parquet file and then uses
    Dask for reading data in chunks.
- If you choose to plot all active sensors (using the `-s all` option):
    - If the -sp flag is not set, the script will plot each sensor in a dedicated browser tab
        one after the other.
        To display the next sensor plot is necessary to press CTRL+C in the terminal.
    - If the -sp flag is set, the script will save each sensor plot as an
        HTML file in the 'plots' folder and display all the plots simultaneously in the browser
        (one tab for each sensor).
...
if missing_packages:
    ...
    for package in missing_packages:
        log.warning(" - (%s) %s", package, missing_packages[package])
...
def hsd_plot_large(
    ...
    """Plot data from acquisition folders, supporting large datasets.
    ...
    labeled : bool
        If True, plot includes acquisition labels where available.
    subplots : bool
        If True, create multiple subplots for multi-dimensional sensors.
    save_plots : bool
        If True, save plots to HTML files instead of launching Dash.
    custom_device_model : tuple[int, int, str] | None
        Optional custom DTDL (board_id, fw_id, json_path).
    debug : bool
        Enable timestamp recovery checks and data consistency debugging.
    """
    ...
    while plot_flag:
        ...
        if sensor_name == '':
            ...
            if component is not None:
                ...
                plot(hsd, component, start_time, end_time, label, subplots, save_plots,
                    raw_data, acq_folder)
            else:
                break
        ...
        # If a specific sensor name is provided, plot only that component
            ...
            if component is not None:
                ...
                log.error('No "%s" Component to plot found in your' \
...
def plot(
    ...
    """Render a plot for a specific component, optionally saving as HTML.
    ...
    label : str | None
        Label class to visualize as time regions.
    subplots : bool
        Create one subplot per data column (except Time).
    save_plots : bool
        Save interactive HTML files instead of launching a Dash app.
    raw_data : bool
        Use raw data values (no sensitivity applied).
    acq_folder : str
    ...
    try:
        ...
        if not os.path.exists(file_path):
            HSDatalog.convert_dat_to_xsv(hsd, component, start_time, end_time, labeled,
                                        raw_data, acquisition_path, "PARQUET")
