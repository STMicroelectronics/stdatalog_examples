...
It supports various options for customizing the data conversion, including selecting specific
sensors, setting time ranges, specifying output formats, aggregating exported data and more.
...
from stdatalog_core.HSD_utils.exceptions import MissingDeviceModelError, \
    MissingISPUOutputDescriptorException, MissingTagsException
...
def hsd_toUnico(
    ...
    """Convert acquisition data to ST MEMS Studio (Unico) formats.
    ...
    output_folder : str | None
        Destination folder. Defaults to `<acq_folder>_Exported` if None.
    sensor_name : str
        Sensor name to export, or 'all' for all active sensors.
    start_time : int
        Start time in seconds for conversion.
    end_time : int
        End time in seconds for conversion (-1 to the end).
    use_datalog_tags : bool
        Include annotations (if present) in output.
    out_format : str | None
        Output format: TXT | CSV | TSV. Defaults to CSV.
    raw_data : bool
        Use raw data values (no sensitivity applied).
    with_untagged : bool
        Also export untagged data sections (split per tags mode).
    with_timestamps : bool
        Include timestamps column in the output files.
    custom_device_model : tuple[int, int, str] | None
        Optional custom DTDL (board_id, fw_id, json_path).
    aggregation : str | None
        Aggregation strategy: 'single_file' or 'split_per_tags'.
    columns_labels : str
        Column naming convention: 'default' or 'mlc_tool'.
    chunk_size : int
        Number of samples per processing chunk.
    debug : bool
        (Unused) Enable debug mode to check for corrupted data and timestamps.
    """

    _ = debug  # Unused parameter
    ...
    while df_flag:
        if aggregation is not None:
            convert_aggregated_data(hsd, aggregation, start_time, end_time, use_datalog_tags,
                                    output_folder, out_format, columns_labels, with_timestamps,
                                    with_untagged, raw_data, chunk_size)
            df_flag = False
        else:
            ...
            if sensor_name == 'all':
                ...
                for component in component_list:
                    ...
                    convert_data(hsd, sensor, start_time, end_time, use_datalog_tags,
                                output_folder, out_format, columns_labels, with_timestamps,
                                raw_data, chunk_size)
                df_flag = False
            # If a specific sensor name is provided, process only that component
                ...
                if component is not None:
                    ...
                    log.error("No \"%s\" Component found in your" \
                        " Device Configuration file.", sensor_name)

                df_flag = False

# Define a helper function to get the sensor component
def __get_sensor_comp(hsd, component):
    """Resolve the sensor component from a generic component mapping.
    ...
    component : dict
        Component mapping as returned by HSDatalog APIs.

    Returns
    -------
    dict
        Filtered component list that matches the sensor name.
    """
...
def convert_aggregated_data(
    ...
    output_folder,
    out_format,
    columns_labels="default",
    with_timestamps=False,
    with_untagged=False,
    raw_data=False,
    chunk_size=HSDatalog.DEFAULT_SAMPLES_CHUNK_SIZE,
):
    """Convert and aggregate data across sensors according to strategy.
    ...
    start_time : int
        Start time in seconds for conversion.
    end_time : int
        End time in seconds for conversion.
    use_datalog_tags : bool
        Include annotations (if present) in output.
    output_folder : str
        Destination folder for exported files.
    out_format : str
        Output format: TXT | CSV | TSV.
    columns_labels : str
        Column naming convention: 'default' or 'mlc_tool'.
    with_timestamps : bool
        Include timestamps column in output files.
    with_untagged : bool
        Also export untagged data sections (split per tags mode).
    raw_data : bool
        Use raw data values (no sensitivity applied).
    chunk_size : int
        Number of samples per processing chunk.
    """
    HSDatalog.convert_dat_to_unico_aggregated(hsd, aggregation, start_time, end_time,
                                            use_datalog_tags, output_folder, out_format,
                                            columns_labels, with_timestamps, with_untagged,
                                            raw_data, chunk_size)
...
def convert_data(
    ...
    """Convert data for a single sensor to Unico-compatible files.
    ...
    hsd : Any
        HSDatalog instance for the acquisition.
    sensor : dict
        Sensor component descriptor to convert.
    start_time : int
        Start time in seconds for conversion.
    end_time : int
        End time in seconds for conversion.
    ...
    try:
        ...
        df = HSDatalog.convert_dat_to_unico(hsd, sensor, start_time, end_time, use_datalog_tags,
                                            output_folder, out_format, columns_labels,
                                            with_timestamps, raw_data, chunk_size)
        return df
    except MissingISPUOutputDescriptorException as ispu_err:
        # Handle missing ISPU output descriptor exception
        log.error("%s", ispu_err)
        log.error("Copy the right ISPU output descriptor file in your \"%s\" acquisition" \
                " folder renamed as \"ispu_output_format.json\"", hsd.get_acquisition_path())
    except MissingTagsException as tags_err:
        log.error("%s", tags_err)
        log.error("Please check the tags field in your \"%s\" file.",
                os.path.join(hsd.get_acquisition_path(),'acquisition_info.json'))
        quit()
